...
const SongManager: React.FC = () => {
  ...
  const filteredSongs = songs.filter((s) =>
    (s[filterField] as string).toLowerCase().includes(filterText.toLowerCase())
  );
  ...
  const stats = [
    ...
    { icon: "🎧", value: Object.entries(
      songs.reduce((acc: Record<string, number>, s) => {
        acc[s.Genre] = (acc[s.Genre] || 0) + 1;
        return acc;
      }, {})
    ).sort((a, b) => b[1] - a[1])[0]?.[0] || "-", label: "Most Common Genre", color: "green" },
  ];
  ...
  const artistStats = Object.values(
    songs.reduce((acc: Record<string, { artist: string; songs: number; albums: Set<string> }>, s) => {
      if (!acc[s.Artist]) acc[s.Artist] = { artist: s.Artist, songs: 0, albums: new Set() };
      acc[s.Artist].songs += 1;
      acc[s.Artist].albums.add(s.Album);
      return acc;
    }, {})
  ).map(a => ({ ...a, albums: a.albums.size }));
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center bg-gray-50 rounded-2xl shadow-md p-6 gap-4 relative">
        ...
        <h1 className="flex items-center text-2xl font-bold text-black gap-3">
          <span className="text-purple-600 text-3xl">🎵</span> Music Catalog Manager
        </h1>
        <div className="flex gap-3 flex-wrap">
          <button
            className="bg-blue-600 text-white px-4 py-2 rounded-xl shadow-md transition font-medium flex items-center justify-center gap-2"
            onClick={handleAddNew}
          >
            {showForm ? "Close Form" : "+ Add New Song"}
          </button>
          <button
            className="bg-gray-200 text-black px-3 py-2 rounded-xl shadow-md transition font-medium flex items-center justify-center gap-2"
            onClick={handleJumpToStats}
          >
            📊 Statistics
          </button>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 bg-gray-50 p-4 rounded-xl shadow-md">
        ...
        <input
          ...
          className="flex-1 border rounded-lg px-4 py-2 shadow-inner focus:outline-none focus:ring-2 focus:ring-blue-300"
        />
      </div>
      ...
      <div className="space-y-2">
        {filteredSongs.length === 0 ? (
          <p className="text-black text-center">No songs found.</p>
        ) : (
            ...
            <div key={song._id} className={`flex flex-col md:flex-row justify-between items-center rounded-2xl shadow-sm p-4 gap-4 ${index % 2 === 0 ? "bg-gray-50" : "bg-gray-100"}`}>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-black flex-1 w-full">
                {["Title", "Artist", "Album", "Genre"].map((field) => (
                  <div key={field}>
                    <div className="text-xs uppercase font-bold text-black/70">{field}</div>
                    <div className="font-medium text-black">{song[field as keyof Song]}</div>
                  </div>
                ))}
              </div>
              <div className="flex gap-2 mt-2 md:mt-0 w-48">
                <button
                  className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-xl shadow-sm hover:bg-blue-700 transition font-medium"
                  onClick={() => handleEdit(song)}
                >
                  Edit
                </button>
                <button
                  className="flex-1 bg-red-600 text-white px-4 py-2 rounded-xl shadow-sm hover:bg-red-700 transition font-medium"
                  onClick={() => setDeleteSongId(song._id ?? null)}
                >
                  Delete
                </button>
              </div>
      ...
      <div ref={statsRef} className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
        {stats.map(({ icon, value, label, color }) => (
          <motion.div key={label} whileHover={{ scale: 1.05 }} className={`relative bg-${color}-100 border border-${color}-300 rounded-xl shadow-sm p-4 flex flex-col items-center text-center`}>
      ...
      <div className="flex justify-center mt-4">
        <button
          className="bg-gray-200 text-black px-6 py-2 rounded-full shadow-sm hover:bg-gray-300 transition font-medium"
              ...
              <div key={title}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {Object.entries(data).map(([key, val]) => (
                    <motion.div key={key} whileHover={{ scale: 1.03 }} className={`bg-${color}-100 border border-${color}-300 rounded-xl shadow-sm p-2 flex flex-col items-center justify-center text-black text-sm`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl shadow-lg w-80">
            ...
            <div className="flex justify-between gap-3">
              <button
                className="flex-1 px-5 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 transition font-medium"
                onClick={() => { dispatch(deleteSongRequest(deleteSongId)); setDeleteSongId(null); }}
              >
                Yes
              </button>
              <button
                className="flex-1 px-5 py-2 bg-gray-200 text-black rounded-xl hover:bg-gray-300 transition font-medium"
